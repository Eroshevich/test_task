 ...
 * (Bundle\BlogBundle\Service\PostController::indexAction).
 *
...
class ControllerNameParser
{
    ...
    public function parse($controller)
    {
        ...
        foreach ($allBundles as $b) {
            $try = $b->getNamespace().'\\Service\\'.$controller.'Service';
            if (class_exists($try)) {
    ...
    {
        if (0 === preg_match('#^(.*?\\\\Service\\\\(.+)Service)::(.+)Action$#', $controller, $match)) {
